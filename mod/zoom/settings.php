...
if ($ADMIN->fulltree) {
    ...
    if (!CLI_SCRIPT && $PAGE->url == $CFG->wwwroot . '/' . $CFG->admin . '/settings.php?section=modsettingzoom') {
        ...
        try {
            ...
            $service->get_user($config->zoomemail);
    ...
    $zoomemail = new admin_setting_configtext('mod_zoom/zoomemail', get_string('zoomemail', 'mod_zoom'),
            get_string('zoomemail_desc', 'mod_zoom'), '', PARAM_NOTAGS);
    $settings->add($zoomemail);
