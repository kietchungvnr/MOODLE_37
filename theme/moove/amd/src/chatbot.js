define(["jquery", "core/config", "core/str", "core/notification"], function($, Config, Str, Notification) {
    ...
    function helpSendMessageHTML(text) {
        var html = '<div class="help-chat"><div class="tpl-text-response-wrapper" data-type="help"><div class="tpl-text-response"><ul class="list-group"><div class="help-content-chatbot"><li class="list-group-item">Xem danh sách khóa học</li><li class="list-group-item">Xem danh sách sách khóa giảng</li><li class="list-group-item">Xem danh sách sách điểm của các khóa học</li><li class="list-group-item">Xem sự kiện sắp hết hạn</li><li class="list-group-item">Xem khóa học sắp hết hạn</li></div></ul></div></div></div></div>';
        return html;
    }
    ...
    var callAjax = (script, settings) => {
        ...
        .then((resp) => {
            ...
            var msg = 'Xin chào ' + fullName + ', tôi là con BOT "ngu". Hi Hi';
    ...
    function chatbotResponse(text) {
        ...
        switch(text) {
            ...
            case 'hi':
                botMessage = botResponseHTML('Ờ hi 😊');
                break;
            case 'Xem danh sách khóa học':
                botMessage = botResponseHTML('Ờ hi 2 😊');
                break;
            case 'Help':
                var helpMessage = botResponseHTML('Tôi là con BOT ngu nên chỉ chọn các lựa chọn dưới đây nha!');
