    ...
    Y.extend(FileManagerHelper, Y.Base, {
        ...
        initializer: function(options) {
            ...
            this.selectui = new M.core.dialogue({
                draggable    : true,
                headerContent: '<h5 id="' + labelid +'" class="headerpopup"></h5>',
        ...
        print_path: function() {
            ...
            if (p && p.length!=0 && this.viewmode != 2) {
                for(var i = 0; i < p.length; i++) {
                    ...
                    if(i > 0) {
                        pathicon = Y.Node.create('<i class="fa fa-angle-right fp-margin" aria-hidden="true"></i>');
                    }
                    
                    this.pathbar.appendChild(pathicon);
        ...
        setup_select_file: function() {
            ...
            selectnode.one('.fp-file-update').on('click', function(e) {
                e.preventDefault();
                this.update_file();
                // Custom by Vũ: Phân quyền trong tài liệu hệ thống
                // var fileinfo = this.selectui.fileinfo;
                // if(fileinfo.caneditfilesystem == true || fileinfo.caneditfoldersystem == true) {
                //     this.update_file();
                // }
            }, this);
            ...
            selectnode.one('.fp-file-delete').on('click', function(e) {
                ...
                switch(isfolder) {
                    case true: 
                        if(fileinfo.candeletefoldersystem == true) {
                            ...
                            // dialog_options.message = M.util.get_string('noconfirmdeletefolder', 'theme_moove');
                            dialog_options.message = 'Không có quyền xóa thư mục';
                            this.selectui.hide(); // TODO remove this after confirm dialog is replaced with YUI3
                            this.show_confirm_dialog(dialog_options); 
                        }
                        break;
                    case false:
                        if(fileinfo.candeletefilesystem == true) {
                            dialog_options.callback = function(params) {
                                //selectnode.addClass('loading');
                                this.request({
                                    action: 'delete',
                            ...
                            // dialog_options.message = M.util.get_string('noconfirmdeletefile', 'theme_moove');
                            dialog_options.message = 'Không có quyền xóa tập tin';
                            this.selectui.hide(); // TODO remove this after confirm dialog is replaced with YUI3
                            this.show_confirm_dialog(dialog_options); 
                        }
                        break;
        ...
        select_file: function(node) {
            ...
            Y.one('#fm-dialog-label_'+selectnode.get('id')).setContent(Y.Escape.html(nodename));
            // show panel
            this.selectui.show();
