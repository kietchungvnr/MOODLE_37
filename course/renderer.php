...
class core_course_renderer extends plugin_renderer_base {
    ...
    /**
     * Custom by Vũ: Clone func để view availability cho chế độ focusmod trong khóa học
     * Renders HTML to show course module availability information (for someone who isn't allowed
     * to see the activity itself, or for staff)
     *
     * @param cm_info $mod
     ...
     * @return string
     */
    public function course_section_cm_availability_focusmod(cm_info $mod, $displayoptions = array()) {
        global $CFG, $OUTPUT;
        $output = '';
        if (!$mod->is_visible_on_course_page()) {
            return $output;
        ...
        if (!$mod->uservisible) {
            ...
            if (!empty($mod->availableinfo)) {
                ...
                $output = $this->availability_info($formattedinfo, 'isrestricted text-muted pb-2 pt-2 pl-3');
            }
            return $output;
        }
        // this is a teacher who is allowed to see module but still should see the
