...
class UserController extends BaseController {
	...
	public function create_and_update($request, $response, $args) {
	    ...
	    $usernamearr = explode(',', $this->data->username);
	    $fullnamearr = explode(',', $this->data->fullname);
	    $emailarr = explode(',', $this->data->email);
	    $phonearr = explode(',', $this->data->phone);
	    foreach ($usercodearr as $key => $usercode) {
	    	$parts = explode(" ", trim($fullnamearr[$key]));
			if(count($parts) > 1) {
			...
			// }
			$userid = find_usercode_by_code(trim($usercode));
			$usernew = new stdClass();
			$usernew->course = 1;
			$usernew->username = trim(strtolower($usernamearr[$key]));
			$usernew->usercode = trim($usercode);
			$usernew->auth = 'manual';
			$usernew->suspended = '0';
			...
			$usernew->email = trim($emailarr[$key]);
			$usernew->maildisplay = 2;
			$usernew->country = 'VN';
			$usernew->lang = 'vi';
			$phone = trim($phonearr[$key]);
			if(isset($phone))
				$usernew->phone1 = trim($phonearr[$key]);
			if($userid) {
				$usernew->id = $userid;
	        ...
	        if(!empty($err)) {
	        	$this->resp->error = true;
	        	$this->resp->message['info'] = 'Dữ liệu lỗi cần kiểm tra lại';
	        	$this->resp->data[] = $err;
	        } else {
	        	if(!$userid) {
