...
class subjectexam_form extends moodleform {
    ...
    function definition () {
        ...
        $mform->addElement('text','name', get_string('name', 'local_newsvnr'),'maxlength="200" size="50"');
        $mform->addRule('name', get_string('required'), 'required', null, 'server');
        $mform->setType('name', PARAM_TEXT);
        $mform->addElement('text', 'code', get_string('code', 'local_newsvnr'), 'maxlength="50" size="50"');
        $mform->addRule('code', get_string('required'), 'required', null, 'client');
        $mform->setType('code', PARAM_TEXT);
        $mform->addElement('text','shortname', get_string('shortname', 'local_newsvnr'),'maxlength="200" size="50"');
        $mform->addRule('shortname', get_string('required'), 'required', null, 'server');
        $mform->setType('shortname', PARAM_TEXT);
        $choices = array();
        $choices['0'] = get_string('hide');
        $choices['1'] = get_string('show');
        $mform->addElement('select', 'visible', get_string('visible', 'local_newsvnr'), $choices);
        $mform->setDefault('visible', $choices['1']);
