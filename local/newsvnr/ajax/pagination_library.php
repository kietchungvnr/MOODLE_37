	...
	function getAllCourseNewsPageLinks($count,$href) {
		...
		if($pages>1) {
				...
				$output = $output . '<span class="link disabled">«</span>';
					...
					$output = $output . '<a class="link" onclick="pageLibrary(\'' . $href . ($_GET["page"]-1) . '&folderid='   . $id . '\')" >«</a>';
				...
				if(($_GET["page"]-3)>0) {
					if($_GET["page"] == 1)
						...
						$output = $output . '<a class="link" onclick="pageLibrary(\'' . $href . '1&folderid='.$id.'\')" >1</a>';
				}
				if(($_GET["page"]-3)>1) {
				...
				for($i=($_GET["page"]-2); $i<=($_GET["page"]+2); $i++)	{
					...
					if($_GET["page"] == $i)
						...
						$output = $output . '<a class="link" onclick="pageLibrary(\'' . $href . $i  .'&folderid='.$id.'\')" >'.$i.'</a>';
				}
