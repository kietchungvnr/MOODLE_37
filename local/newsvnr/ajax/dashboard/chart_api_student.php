...
switch ($action) {
    case 'quizchart':
        ...
        foreach ($listcourse as $value) {
            ...
            $obj->rank      = get_rank_student_incourse($value->id, $USER->id);
            $obj->y         = ($grade != false) ? (int) $grade->gradefinal : 0;
            $obj->drilldown = $value->id;
            $avggrade[]     = $obj;
        }
        // foreach ($listcourse as $course) {
        //     $listgrade = get_grade_module_incourse($course->id, $USER->id);
        //     $obj       = new stdClass();
        //     $obj->name = 'grade';
        //     $obj->id   = $course->id;
        //     foreach ($listgrade as $grade) {
        //         $objquiz     = [];
        //         $objquiz[]   = $grade->itemname;
        //         $objquiz[]   = ((int) $grade->finalgrade * 10) / ((int) $grade->grademax);
        //         $obj->data[] = $objquiz;
        //     }
        //     $quizgrade[] = $obj;
        // }
        // Điểm trung bình của khóa học
        // $a = get_rank_student_incourse($value->id);
        $data['avggradecourse'] = [3, 0, 3, 0, 3];
    ...
    case 'homework':
        ...
        break;
    case 'myquiz':
        $myquiz = $DB->get_records_sql("SELECT ROW_NUMBER() OVER (ORDER BY q.id) AS RowNum,c.id,q.name,gg.finalgrade,gg.userid,q.timeclose
